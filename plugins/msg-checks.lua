...
local lang = redis:get(hash)
  ...
  tdcli.changeChatMemberStatus(chat, our_id, 'Left', dl_cb, nil)
   ...
   if lock_photo then
	...
	if lock_contact then
		lock_contact = lock_contact
	else
		lock_contact = 'no'
	end
	if lock_inline then
	...
	if lock_keyboard then
		lock_keyboard = lock_keyboard
	else
		lock_keyboard = 'no'
	end
	if lock_forward then
		lock_forward = lock_forward
	else
		lock_forward = 'no'
	end
	if lock_location then
		lock_location = lock_location
	else
		lock_location = 'no'
	end
   if lock_document then
	...
	if  lock_video then
		lock_video = lock_video
	else
		lock_video = 'no'
	end
	if lock_tgservice then
		lock_tgservice = lock_tgservice
	else
		lock_tgservice = 'no'
	end
